...
export function ReclamoDetalle({ id }: { id: string }) {
  ...
  useEffect(() => {
    // 🔹 Acá hay que traer el reclamo desde la API con fetch(`/api/reclamos/${id}`)
    setReclamo({
      id,
      titulo: `Reclamo ${id}`,
      descripcion: `Aquí iría la información detallada del reclamo ${id}, traída desde la base de datos.`,
    });
  }, [id]);
  ...
  const actualizarEstado = async (estado: string) => {
    try {
      ...
      await fetch(`/api/reclamos/${id}/estado`, {
        method: "POST",
